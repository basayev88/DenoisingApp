...
# State untuk preview citra
if "preview_pairs" not in st.session_state:
...
if st.button("🚀 Start Denoising and Prepare ZIP", type="primary", disabled=start_disabled):
    # reset preview setiap kali proses baru
    st.session_state["preview_pairs"] = []
    ...
    with validation_status:
        ...
        def process_one(img_norm, dcm_obj, fname):
            try:
                ...
                # simpan maksimal 5 pasangan untuk preview
                if len(st.session_state["preview_pairs"]) < 5:
                    noisy_img = (img_norm * 255.0).clip(0, 255).astype("uint8")
                    den_img = (denoised * 255.0).clip(0, 255).astype("uint8")

                    noisy_pil = Image.fromarray(noisy_img)
                    den_pil = Image.fromarray(den_img)

                    st.session_state["preview_pairs"].append(
                        {
                            "fname": fname,
                            "noisy": noisy_pil,
                            "denoised": den_pil,
        ...
        if st.session_state.get("preview_pairs"):
            ...
            for pair in st.session_state["preview_pairs"]:
                ...
                c1, c2 = st.columns(2)
                with c1:
                    st.caption("Before (Noisy)")
                    st.image(pair["noisy"], use_container_width=True)
                with c2:
                    st.caption("After (Denoised)")
                    st.image(pair["denoised"], use_container_width=True)
...
with col2:
    ...
    st.markdown("---")
    st.markdown("Give feedback to: ism22@tpu.ru")
